...
export default class datePicker extends EventEmitter {
  ...
  value(value = null) {
    ...
    // Get
    let string = this._isValidDate(this.start)
  ...
  refresh() {
    ...
    months.forEach((month) => {
      ...
      if (
        month.dataset.month ===
        dateFns.format(this._visibleDate, 'MM', { locale: this.locale })
      ) {
    ...
    years.forEach((year) => {
      ...
      if (
        year.dataset.year ===
        dateFns.format(this._visibleDate, 'yyyy', { locale: this.locale })
      ) {
    ...
    if (
      this.min &&
      dateFns.differenceInCalendarMonths(this._visibleDate, this.min) === 0
    ) {
      this._togglePreviousButton(false)
    } else {
    ...
    if (
      this.max &&
      dateFns.differenceInCalendarMonths(this._visibleDate, this.max) === 0
    ) {
      this._toggleNextButton(false)
    } else {
    ...
    this._ui.navigation.month.innerHTML = dateFns.format(
      this._visibleDate,
      this.options.formats.navigationMonth,
      { locale: this.locale }
    )
    this._ui.navigation.year.innerHTML = dateFns.format(
      this._visibleDate,
      this.options.formats.navigationYear,
      { locale: this.locale }
    )
  ...
  _init() {
    ...
    this.disabledDates = type.isFunction(this.options.disabledDates)
    ...
    this.disabledWeekDays = type.isString(this.options.disabledWeekDays)
      ...
      : []
    this.highlightedDates = type.isFunction(this.options.highlightedDates)
    ...
    if (Array.isArray(this.options.highlightedDates)) {
      for (let i = 0; i < this.options.highlightedDates.length; i++) {
        this.highlightedDates.push(
          utils.newDate(
            this.options.highlightedDates[i],
            this.format,
            'yyyy-MM-dd'
          )
        )
    ...
    this._date = {
      start: utils.newDate(this.options.startDate, this.format, 'yyyy-MM-dd'),
        ...
        ? utils.newDate(this.options.endDate, this.format, 'yyyy-MM-dd')
        : undefined
    }
    ...
    const days = new Array(dateFns.differenceInDays(end, start) + 1)
      ...
      .map((s, i) => {
        ...
        const isInRange =
          ...
          this.end &&
          dateFns.isWithinInterval(theDate, {
            start: dateFns.startOfDay(this.start),
            end: dateFns.endOfDay(this.end)
          })
        ...
        if (Array.isArray(this.disabledDates)) {
          for (let j = 0; j < this.disabledDates.length; j++) {
            if (
              theDate.toDateString() === this.disabledDates[j].toDateString()
            ) {
              isDisabled = true
            }
          }
        ...
        if (Array.isArray(this.highlightedDates)) {
          for (let j = 0; j < this.highlightedDates.length; j++) {
            if (
              theDate.toDateString() === this.highlightedDates[j].toDateString()
            ) {
              isHighlighted = true
            }
          }
