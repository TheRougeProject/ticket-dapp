...
export default class timePicker extends EventEmitter {
  constructor(options = {}) {
    ...
    this.onPreviousHourStartTimePicker =
      this.onPreviousHourStartTimePicker.bind(this)
    this.onNextHourStartTimePicker = this.onNextHourStartTimePicker.bind(this)
    this.onPreviousMinuteStartTimePicker =
      this.onPreviousMinuteStartTimePicker.bind(this)
    this.onNextMinuteStartTimePicker =
      this.onNextMinuteStartTimePicker.bind(this)
    this.onPreviousHourEndTimePicker =
      this.onPreviousHourEndTimePicker.bind(this)
    this.onNextHourEndTimePicker = this.onNextHourEndTimePicker.bind(this)
    this.onPreviousMinuteEndTimePicker =
      this.onPreviousMinuteEndTimePicker.bind(this)
  ...
  _init() {
    ...
    this._time = {
      start: dateFns.addHours(dateFns.startOfToday(), 8),
      end: dateFns.addHours(dateFns.startOfToday(), 17)
    }

    this.start =
      utils.newDate(this.options.startTime, this.format, 'HH:mm') ||
      dateFns.addHours(dateFns.startOfToday(), 8)
    this.end = this.options.isRange
      ? utils.newDate(this.options.endTime, this.format, 'HH:mm')
      : dateFns.addHours(dateFns.startOfToday(), 17)
  ...
  _bindEvents() {
    this._clickEvents.forEach((clickEvent) => {
      this._ui.start.hours.previous.addEventListener(
        clickEvent,
        this.onPreviousHourStartTimePicker
      )
      this._ui.start.hours.next.addEventListener(
        clickEvent,
        this.onNextHourStartTimePicker
      )
      this._ui.start.minutes.previous.addEventListener(
        clickEvent,
        this.onPreviousMinuteStartTimePicker
      )
      this._ui.start.minutes.next.addEventListener(
        clickEvent,
        this.onNextMinuteStartTimePicker
      )

      if (this.options.isRange) {
        this._ui.end.hours.previous.addEventListener(
          clickEvent,
          this.onPreviousHourEndTimePicker
        )
        this._ui.end.hours.next.addEventListener(
          clickEvent,
          this.onNextHourEndTimePicker
        )
        this._ui.end.minutes.previous.addEventListener(
          clickEvent,
          this.onPreviousMinuteEndTimePicker
        )
        this._ui.end.minutes.next.addEventListener(
          clickEvent,
  ...
  _select(time = undefined, emit = true) {
    ...
    if (
      ...
      (!this._isValidTime(this.start) ||
        (this._isValidTime(this.start) && this._isValidTime(this.end)))
    ) {
      this.start = time
      ...
      this.start = time
      this.end = dateFns.addHours(dateFns.startOfToday(), 17)
      if (emit) this.emit('select', this)
    }
  }
  ...
  set start(time) {
    if (!time) {
      this._time.start = dateFns.addHours(dateFns.startOfToday(), 8)
      return this
    }
  ...
  set end(time) {
    if (!time) {
      this._time.end = dateFns.addHours(dateFns.startOfToday(), 17)
      return this
    }
  ...
  onPreviousMinuteStartTimePicker(e) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        this._ui.start.minutes.number.innerText = dateFns.format(
          this.start,
          'mm'
        )
        ...
        if (
          dateFns.format(this.start, 'HH') !== this._ui.start.hours.input.value
        ) {
          this._ui.start.hours.number.innerText = dateFns.format(
            this.start,
            'HH'
          )
          this._ui.start.hours.input.value = dateFns.format(this.start, 'HH')
  ...
  value(value = null) {
    ...
    if (value) {
      if (this.options.isRange) {
        ...
        if (Array.isArray(value)) {
          if (value.length)
            this.start = utils.newDate(value[0], this.format, 'yyyy-MM-dd')
          if (value.length === 2)
            this.end = utils.newDate(value[1], this.format, 'yyyy-MM-dd')
        }
      } else {
    ...
    // Get
    let string = this._isValidTime(this.start)
      ? dateFns.format(this.start, this.format, { locale: this.locale })
      : ''
